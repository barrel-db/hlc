...
> :heavy_exclamation_mark: hlc is not stored as gen_server anymore allowing you to reuse it more easily in your own
> project.
...
:raised_hand: A clock is not locked, you need to make sure that only one user can update it at a time (using the `now/1`
or `update/2` functions.
